...
public class LookbackCalculation {
		...
		for(int i=length; i>0; i--){
			...
			for(Integer integer:masks){
				...
				if(i==length){
					...
					//binaryString.lastIndexof("1") is the time 1 was in the sequence. ex: THTT, = 1
					//Only looks at the last three characters in the first OR case. ex: TTHH, only THH is viewed
					//if binaryString doesn't have 1 OR (H is the last character AND doesnt have any other H's after searching the rest of the string) search starts at the second position backwards after the first H is found at the end. 
					//the FIRST COND only handles the first string of flips, the SECOND COND together handles the second string of slips (TTTH)

// ***************** TOOK OUT IF, NOT NEEDED. *****************************************************************************************************					
//					if(!binaryString.contains("1") || (binaryString.lastIndexOf("1")==binaryString.length()-1 && binaryString.lastIndexOf("1", binaryString.length()-2)==-1)){
//						
//						//startingValue is used here because it is the default amount for these Tails parts since they wont go higher than default
//						double tmpValue = startingValue-convertBinStrToDouble(binaryString);
//						tmpValue = (double)Math.round(tmpValue * 100d) / 100d;
//						
//						//putting the tmp value at this binaryString index of the array
//						vValuesMaps.get(i-1).put(binaryString, tmpValue+"");
//						System.out.println("V" + i + "(" + convertBinStrToCoinFlips(binaryString) + ")" + " =  " + tmpValue);
//						continue;
//					}
					...
					double tmpValue = convertBinStrToDoubleFirstPart(firstPart)-convertBinStrToDouble(binaryString);
					tmpValue = (double)Math.round(tmpValue * 100d) / 100d;
				...
				//For the second half equations. if n=3, then the 2's, 1's
					...
					double tmpValue = (1/(1+rate))*(p*Double.parseDouble(vValuesMaps.get(i).get(firstPart))+q*Double.parseDouble(vValuesMaps.get(i).get(secondPart)));
					tmpValue = (double)Math.round(tmpValue * 100d) / 100d;
